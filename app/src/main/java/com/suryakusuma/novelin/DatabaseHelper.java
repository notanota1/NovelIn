...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public DatabaseHelper(Context context) {
        super(context, DATABASE_NAME, null, 3); // Naikkan versi ke 3
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + TABLE_USERS + " (ID INTEGER PRIMARY KEY AUTOINCREMENT, USERNAME TEXT UNIQUE, PASSWORD TEXT)");
        db.execSQL("CREATE TABLE " + TABLE_SAVED + " (ID INTEGER PRIMARY KEY AUTOINCREMENT, USERNAME TEXT, TITLE TEXT, AUTHOR TEXT, DESCRIPTION TEXT, COVER_URL TEXT, COVER_RES INTEGER)");
    }
    ...
    public List<Novel> getFullSavedNovels(String username) {
        ...
        while (cursor.moveToNext()) {
            ...
            if (url != null && !url.isEmpty()) {
                list.add(new Novel(title, author, desc, url, new ArrayList<>()));
            } else {
                list.add(new Novel(title, author, desc, resId, new ArrayList<>()));
            }
        }
        cursor.close();
        return list;
    ...
    // Tetap pertahankan method lama untuk pengecekan status tombol
